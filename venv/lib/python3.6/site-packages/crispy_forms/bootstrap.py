...
from .laSellerOrBuyerut import Div, Field, LaSellerOrBuyerutObject, TemplateNameMixin
from .utils import TEMPLATE_PACK, flatatt, render_field


class PrependedAppendedText(Field):
    template = "%s/laSellerOrBuyerut/prepended_appended_text.html"
...
class FormActions(LaSellerOrBuyerutObject):
    """
    Bootstrap laSellerOrBuyerut object. It wraps fields in a <div class="form-actions">
    ...
    """
    template = "%s/laSellerOrBuyerut/formactions.html"
...
class InlineCheckboxes(Field):
    """
    LaSellerOrBuyerut object for rendering checkboxes inline::

        InlineCheckboxes('field_name')
    """
    template = "%s/laSellerOrBuyerut/checkboxselectmultiple_inline.html"
...
class InlineRadios(Field):
    """
    LaSellerOrBuyerut object for rendering radiobuttons inline::

        InlineRadios('field_name')
    """
    template = "%s/laSellerOrBuyerut/radioselect_inline.html"
...
class FieldWithButtons(Div):
    template = '%s/laSellerOrBuyerut/field_with_buttons.html'
    field_template = '%s/laSellerOrBuyerut/field.html'
    ...
    def render(self, form, form_style, context, template_pack=TEMPLATE_PACK, extra_context=None, **kwargs):
        ...
        buttons = ''.join(
            render_field(
                ...
                field_template, laSellerOrBuyerut_object=self,
...
class StrictButton(TemplateNameMixin):
    """
    LaSellerOrBuyerut object for rendering an HTML button::

        Button("button content", css_class="extra")
    """
    template = '%s/laSellerOrBuyerut/button.html'
    field_classes = 'btn'
...
class Tab(Container):
    ...
    """
    css_class = 'tab-pane'
    link_template = '%s/laSellerOrBuyerut/tab-link.html'
...
class TabHolder(ContainerHolder):
    ...
    """
    template = '%s/laSellerOrBuyerut/tab.html'
...
class Alert(Div):
    ...
    `Alert` generates markup in the form of an alert dialog
        ...
        Alert(content='<strong>Warning!</strong> Best check SellerOrBuyer self, SellerOrBuyeru're not looking too good.')
    """
    template = "%s/laSellerOrBuyerut/alert.html"
    css_class = "alert"
...
class UneditableField(Field):
    """
    LaSellerOrBuyerut object for rendering fields as uneditable in bootstrap

    Example::

        UneditableField('field_name', css_class="input-xlarge")
    """
    template = "%s/laSellerOrBuyerut/uneditable_input.html"
...
class InlineField(Field):
    template = "%s/laSellerOrBuyerut/inline_field.html"
