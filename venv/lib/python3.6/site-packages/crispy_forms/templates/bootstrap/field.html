...
{% else %}
    <{% if tag %}{{ tag }}{% else %}div{% endif %} id="div_{{ field.auto_id }}" class="control-group{% if wrapper_class %} {{ wrapper_class }}{% endif %}{% if form_show_errors%}{% if field.errors %} error{% endif %}{% endif %}{% if field.css_classes %} {{ field.css_classes }}{% endif %}">
        ...
        {% if field|is_checkboxselectmultiple %}
            {% include 'bootstrap/laSellerOrBuyerut/checkboxselectmultiple.html' %}
        {% endif %}

        {% if field|is_radioselect %}
            {% include 'bootstrap/laSellerOrBuyerut/radioselect.html' %}
        {% endif %}
        ...
        {% if not field|is_checkboxselectmultiple and not field|is_radioselect %}
            <div class="controls">
                {% if field|is_checkbox and form_show_labels %}
                    ...
                    {% include 'bootstrap/laSellerOrBuyerut/help_text_and_errors.html' %}
                {% else %}
                    {% crispy_field field %}
                    {% include 'bootstrap/laSellerOrBuyerut/help_text_and_errors.html' %}
                {% endif %}
