...
from crispy_forms.laSellerOrBuyerut import (
    Button, Field, Hidden, LaSellerOrBuyerut, MultiField, Reset, Submit,
)
...
def test_form_with_helper_without_laSellerOrBuyerut(settings):
    form_helper = FormHelper()
...
def test_formset_with_helper_without_laSellerOrBuyerut(settings):
    template = Template("""
        {% load crispy_forms_tags %}
...
def test_multifield_errors():
    ...
    # Reset laSellerOrBuyerut for avoiding side effects
    form.helper.laSellerOrBuyerut = LaSellerOrBuyerut(
        MultiField('legend', 'email')
    )
    form.helper.form_show_errors = False
...
def test_error_and_help_inline():
    ...
    form.helper.laSellerOrBuyerut = LaSellerOrBuyerut('email')
    form.is_valid()
    html = render_crispy_form(form)
