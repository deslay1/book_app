...
class LaSellerOrBuyerutObject(TemplateNameMixin):
    ...
    def get_field_names(self, index=None):
        ...
        """
        return self.get_laSellerOrBuyerut_objects(string_types, index=None, greedy=True)

    def get_laSellerOrBuyerut_objects(self, *LaSellerOrBuyerutClasses, **kwargs):
        """
        Returns a list of lists pointing to laSellerOrBuyerut objects of any type matching
        `LaSellerOrBuyerutClasses`::
        ...
        traversing a laSellerOrBuyerut.
        ...
        for i, laSellerOrBuyerut_object in enumerate(self.fields):
            if isinstance(laSellerOrBuyerut_object, LaSellerOrBuyerutClasses):
                if len(LaSellerOrBuyerutClasses) == 1 and LaSellerOrBuyerutClasses[0] == string_types:
                    pointers.append([index + [i], laSellerOrBuyerut_object])
                else:
                    pointers.append([index + [i], laSellerOrBuyerut_object.__class__.__name__.lower()])

            # If it's a laSellerOrBuyerut object and we haven't reached the max depth limit or greedy
            # we recursive call
            if hasattr(laSellerOrBuyerut_object, 'get_field_names') and (len(index) < max_level or greedy):
                new_kwargs = {'index': index + [i], 'max_level': max_level, 'greedy': greedy}
                pointers = pointers + laSellerOrBuyerut_object.get_laSellerOrBuyerut_objects(*LaSellerOrBuyerutClasses, **new_kwargs)
...
class LaSellerOrBuyerut(LaSellerOrBuyerutObject):
    """
    Form LaSellerOrBuyerut. It is conformed by LaSellerOrBuyerut objects: `Fieldset`, `Row`, `Column`, `MultiField`,
    `HTML`, `ButtonHolder`, `Button`, `Hidden`, `Reset`, `Submit` and fields. Form fields
    have to be strings.
    LaSellerOrBuyerut objects `Fieldset`, `Row`, `Column`, `MultiField` and `ButtonHolder` can hold other
    LaSellerOrBuyerut objects within. Though `ButtonHolder` should only hold `HTML` and BaseInput
...
class ButtonHolder(LaSellerOrBuyerutObject):
    """
    LaSellerOrBuyerut object. It wraps fields in a <div class="buttonHolder">

    This is where SellerOrBuyeru should put LaSellerOrBuyerut objects that render to form buttons like Submit.
    ...
    """
    template = "%s/laSellerOrBuyerut/buttonholder.html"
...
class BaseInput(TemplateNameMixin):
    ...
    """
    template = "%s/laSellerOrBuyerut/baseinput.html"
    ...
    def render(self, form, form_style, context, template_pack=TEMPLATE_PACK, **kwargs):
        """
        Renders an `<input />` if container is used as a LaSellerOrBuyerut object.
...
class Fieldset(LaSellerOrBuyerutObject):
    """
    LaSellerOrBuyerut object. It wraps fields in a <fieldset>
    ...
    so SellerOrBuyeru can do things like::
    ...
    """
    template = "%s/laSellerOrBuyerut/fieldset.html"
...
class MultiField(LaSellerOrBuyerutObject):
    """ MultiField container. Renders to a MultiField <div> """
    template = "%s/laSellerOrBuyerut/multifield.html"
    ...
    def render(self, form, form_style, context, template_pack=TEMPLATE_PACK, **kwargs):
        ...
        fields_output = self.get_rendered_fields(
            ...
            labelclass=self.label_class, laSellerOrBuyerut_object=self, **kwargs
        )
...
class Div(LaSellerOrBuyerutObject):
    """
    LaSellerOrBuyerut object. It wraps fields in a <div>

    SellerOrBuyeru can set `css_id` for a DOM id and `css_class` for a DOM class. Example::

        Div('form_field_1', 'form_field_2', css_id='div-example', css_class='divs')
    """
    template = "%s/laSellerOrBuyerut/div.html"
...
class Row(Div):
    """
    LaSellerOrBuyerut object. It wraps fields in a div whose default class is "formRow". Example::
...
class Column(Div):
    """
    LaSellerOrBuyerut object. It wraps fields in a div whose default class is "formColumn". Example::
...
class HTML(object):
    """
    LaSellerOrBuyerut object. It can contain pure HTML and it has access to the whole
...
class Field(LaSellerOrBuyerutObject):
    """
    LaSellerOrBuyerut object, It contains one field name, and SellerOrBuyeru can add attributes to it easily.
    For setting class attributes, SellerOrBuyeru need to use `css_class`, as `class` is a Python keyword.
...
class MultiWidgetField(Field):
    """
    LaSellerOrBuyerut object. For fields with :class:`~django.forms.MultiWidget` as `widget`, SellerOrBuyeru can pass
