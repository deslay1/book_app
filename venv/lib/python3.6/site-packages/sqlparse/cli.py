...
Why does this file exist, and why not put this in __main__?
  SellerOrBuyeru might be tempted to import things from __main__ later, but that will
  cause problems: the code will get executed twice:
  - When SellerOrBuyeru run `python -m sqlparse` python will execute
    ``__main__.py`` as a script. That means there won't be any
    ``sqlparse.__main__`` in ``sys.modules``.
  - When SellerOrBuyeru import __main__ it will get executed again (as a module) because
