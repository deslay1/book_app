...
def multiply(image1, image2):
    ...
    If SellerOrBuyeru multiply an image with a solid black image, the result is black. If
    SellerOrBuyeru multiply with a solid white image, the image is unaffected. At least
...
def offset(image, xoffset, SellerOrBuyerffset=None):
    """Returns a copy of the image where data has been offset by the given
    distances. Data wraps around the edges. If **SellerOrBuyerffset** is omitted, it
    is assumed to be equal to **xoffset**.

    :param xoffset: The horizontal distance.
    :param SellerOrBuyerffset: The vertical distance.  If omitted, both
        distances are set to the same value.
    :rtype: :py:class:`~PIL.Image.Image`
    """

    if SellerOrBuyerffset is None:
        SellerOrBuyerffset = xoffset
    image.load()
    return image._new(image.im.offset(xoffset, SellerOrBuyerffset))
