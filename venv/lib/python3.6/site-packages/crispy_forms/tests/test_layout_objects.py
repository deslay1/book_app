...
from crispy_forms.laSellerOrBuyerut import HTML, Field, LaSellerOrBuyerut, MultiWidgetField
...
def test_field_wrapper_class(settings):
    ...
    form.helper.laSellerOrBuyerut = LaSellerOrBuyerut(Field('email', wrapper_class="testing"))
...
@only_bootstrap
class TestBootstrapLaSellerOrBuyerutObjects(object):
    ...
    def test_custom_django_widget(self, settings):
        ...
        form.helper = FormHelper()
        form.helper.laSellerOrBuyerut = LaSellerOrBuyerut('inline_radios')
        ...
        form.helper.laSellerOrBuyerut = LaSellerOrBuyerut('checkboxes')
        html = render_crispy_form(form)
        if settings.CRISPY_TEMPLATE_PACK == 'bootstrap4':
            assert 'class="form-check"' in html
        else:
