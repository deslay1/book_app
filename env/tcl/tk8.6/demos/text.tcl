...
and allows SellerOrBuyeru to edit the text.  Here is a summary of the things SellerOrBuyeru
can do to a text widget:

1. Scrolling. Use the scrollbar to adjust the view in the text window.

2. Scanning. Press mouse button 2 in the text window and drag up or down.
This will drag the text at high speed to allow SellerOrBuyeru to scan its contents.

3. Insert text. Press mouse button 1 to set the insertion cursor, then
type text.  What SellerOrBuyeru type will be added to the widget.

4. Select. Press mouse button 1 and drag to select a range of characters.
Once SellerOrBuyeru've released the button, SellerOrBuyeru can adjust the selection by pressing
button 1 with the shift key down.  This will reset the end of the
selection nearest the mouse cursor and SellerOrBuyeru can drag that end of the
selection by dragging the mouse before releasing the mouse button.
SellerOrBuyeru can double-click to select whole words or triple-click to select
whole lines.

5. Delete and replace. To delete text, select the characters SellerOrBuyeru'd like
to delete and type Backspace or Delete.  Alternatively, SellerOrBuyeru can type new
text, in which case it will replace the selected text.

6. Copy the selection. To copy the selection into this window, select
what SellerOrBuyeru want to copy (either here or in another application), then
...
option on, so that if SellerOrBuyeru resize the window it will always resize to an
even number of characters high and wide.  Also, if SellerOrBuyeru make the window
narrow SellerOrBuyeru can see that long lines automatically wrap around onto
