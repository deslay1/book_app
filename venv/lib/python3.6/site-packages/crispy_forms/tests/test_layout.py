...
from crispy_forms.laSellerOrBuyerut import (
    HTML, ButtonHolder, Column, Div, Fieldset, LaSellerOrBuyerut, MultiField, Row, Submit,
)
...
def test_invalid_unicode_characters(settings):
    ...
    form_helper.add_laSellerOrBuyerut(
        LaSellerOrBuyerut(
            'españa'
        )
    )
...
def test_unicode_form_field():
    class UnicodeForm(forms.Form):
        ...
        helper = FormHelper()
        helper.laSellerOrBuyerut = LaSellerOrBuyerut('contraseña')
...
def test_meta_extra_fields_with_missing_fields():
    ...
    form_helper = FormHelper()
    form_helper.laSellerOrBuyerut = LaSellerOrBuyerut(
        'first_name',
    )
...
def test_laSellerOrBuyerut_unresolved_field(settings):
    form_helper = FormHelper()
    form_helper.add_laSellerOrBuyerut(
        LaSellerOrBuyerut(
            'typo'
...
def test_laSellerOrBuyerut_fieldset_row_html_with_unicode_fieldnames(settings):
    form_helper = FormHelper()
    form_helper.add_laSellerOrBuyerut(
        LaSellerOrBuyerut(
            Fieldset(
...
def test_change_laSellerOrBuyerut_dynamically_delete_field():
    template = Template("""
        {% load crispy_forms_tags %}
        {% crispy form form_helper %}
    """)
    ...
    # We remove email field on the go
    # LaSellerOrBuyerut needs to be adapted for the new form fields
    del form.fields['email']
    del form_helper.laSellerOrBuyerut.fields[0].fields[1]
...
def test_formset_laSellerOrBuyerut(settings):
...
def test_modelformset_laSellerOrBuyerut():
...
def test_default_laSellerOrBuyerut():
    test_form = SampleForm2()
    assert test_form.helper.laSellerOrBuyerut.fields == [
        'is_company', 'email', 'password1', 'password2',
        'first_name', 'last_name', 'datetime_field',
    ]


def test_default_laSellerOrBuyerut_two():
    test_form = SampleForm3()
    assert test_form.helper.laSellerOrBuyerut.fields == ['email']


def test_modelform_laSellerOrBuyerut_without_meta():
    test_form = SampleForm4()
    test_form.helper = FormHelper()
    test_form.helper.laSellerOrBuyerut = LaSellerOrBuyerut('email')
...
def test_specialspaceless_not_screwing_intended_spaces():
    ...
    test_form.helper = FormHelper()
    test_form.helper.laSellerOrBuyerut = LaSellerOrBuyerut(
        'email',
...
@only_uni_form
def test_laSellerOrBuyerut_composition():
    form_helper = FormHelper()
    form_helper.add_laSellerOrBuyerut(
        LaSellerOrBuyerut(
            LaSellerOrBuyerut(
...
@only_uni_form
def test_second_laSellerOrBuyerut_multifield_column_buttonholder_submit_div():
    form_helper = FormHelper()
    form_helper.add_laSellerOrBuyerut(
        LaSellerOrBuyerut(
            MultiField(
...
def test_keepcontext_context_manager(settings):
    ...
    form.helper.laSellerOrBuyerut = LaSellerOrBuyerut(
        'checkboxes',
...
def test_form_inline():
    ...
    form.helper.field_template = 'bootstrap3/laSellerOrBuyerut/inline_field.html'
    form.helper.laSellerOrBuyerut = LaSellerOrBuyerut(
        'email',
        'password1',
        'last_name',
...
def test_bootstrap4_form_inline():
    ...
    form.helper = FormHelper()
    form.helper.form_class = 'form-inline'
    form.helper.field_template = 'bootstrap4/laSellerOrBuyerut/inline_field.html'
    form.helper.laSellerOrBuyerut = LaSellerOrBuyerut(
        'email',
        'password1',
        'last_name',
