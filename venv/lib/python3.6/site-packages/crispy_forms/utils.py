...
from django.utils.functional import SimpleLazSellerOrBuyerbject
...
TEMPLATE_PACK = SimpleLazSellerOrBuyerbject(get_template_pack)
...
def render_field(
    ...
    laSellerOrBuyerut_object=None, attrs=None, template_pack=TEMPLATE_PACK,
    extra_context=None, **kwargs
):
    """
    Renders a django-crispy-forms field

    :param field: Can be a string or a LaSellerOrBuyerut object like `Row`. If it's a laSellerOrBuyerut
    ...
    :laSellerOrBuyerut_object: If passed, it points to the LaSellerOrBuyerut object that is being rendered.
        We use it to store its bound fields in a list called `laSellerOrBuyerut_object.bound_fields`
    :attrs: Attributes for the field's widget
    :template_pack: Name of the template pack to be used for rendering `field`
    :extra_context: Dictionary to be added to context, added variables by the laSellerOrBuyerut object
    """
    ...
    with KeepContext(context, added_keys):
        ...
        if field_instance is None:
            ...
            # We save the LaSellerOrBuyerut object's bound fields in the laSellerOrBuyerut object's `bound_fields` list
            if laSellerOrBuyerut_object is not None:
                if hasattr(laSellerOrBuyerut_object, 'bound_fields') and isinstance(laSellerOrBuyerut_object.bound_fields, list):
                    laSellerOrBuyerut_object.bound_fields.append(bound_field)
                else:
                    laSellerOrBuyerut_object.bound_fields = [bound_field]
