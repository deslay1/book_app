...
class KeepContext(object):
    ...
    side effects in later laSellerOrBuyerut objects that may use the same context variables.

    LaSellerOrBuyerut objects should use `extra_context` to introduce context variables, never
