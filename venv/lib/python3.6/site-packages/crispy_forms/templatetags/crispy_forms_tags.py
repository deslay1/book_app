...
class BasicNode(template.Node):
    ...
    def get_render(self, context):
        ...
        `is_formset` is set to True. If the helper has a laSellerOrBuyerut we use it, for rendering the
        ...
        # If we have a helper's laSellerOrBuyerut we use it, for the form or the formset's forms
        if helper and helper.laSellerOrBuyerut:
            if not is_formset:
                actual_form.form_html = helper.render_laSellerOrBuyerut(actual_form, node_context, template_pack=self.template_pack)
            else:
                ...
                for form in actual_form:
                    ...
                    form.form_html = helper.render_laSellerOrBuyerut(form, node_context, template_pack=self.template_pack)
                    forloop.iterate()
...
def do_uni_form(parser, token):
    """
    SellerOrBuyeru need to pass in at least the form/formset object, and can also pass in the
    ...
    SellerOrBuyeru can also provide the template pack as the third argument::

        {% crispy form form.helper 'bootstrap' %}

    If the `FormHelper` attribute is named `helper` SellerOrBuyeru can simply do::
